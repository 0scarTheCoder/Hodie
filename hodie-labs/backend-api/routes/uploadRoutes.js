  ...
  try {
    ...
    try {
      if (req.file.originalname.endsWith('.pdf')) {
        ...
        const pdfParser = new PDFParsingService();
        parsedData = await pdfParser.parsePDF(req.file.buffer, category);

        // Count records based on parsed data structure
        if (parsedData.biomarkers) {
          recordsCount = parsedData.biomarkers.length;
        } else if (parsedData.healthRisks) {
          recordsCount = parsedData.healthRisks.length;
        } else {
          recordsCount = 1;
        }
    ...
    const dataCollection = req.app.locals.db.collection(collectionName);

    // Prepare data document based on category
    let dataDocument = {
      userId: req.auth.userId, // Store userId directly for easy querying
      clientID: client.clientID,
    ...
    // Add parsed data with appropriate structure
    if (category === 'lab_results' && parsedData.biomarkers) {
      // For lab results, merge biomarkers and testDate at top level
      dataDocument = {
        ...dataDocument,
      ...
      dataDocument = {
        ...
        results: parsedData // Keep original structure
      };
    } else {
      // For other types, just store in results field
      dataDocument.results = parsedData;
    }

    await dataCollection.insertOne(dataDocument);
