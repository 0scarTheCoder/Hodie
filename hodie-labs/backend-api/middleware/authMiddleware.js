...
/**
 * Verify Firebase JWT token (simplified - in production, use Firebase Admin SDK)
 */
function verifyFirebaseToken(token) {
  return new Promise((resolve, reject) => {
    // For Firebase tokens, we'll decode without verification for now
    // In production, use Firebase Admin SDK: admin.auth().verifyIdToken(token)
    try {
      const decoded = jwt.decode(token);
      if (!decoded) {
        reject(new Error('Invalid Firebase token'));
        return;
      }
      resolve(decoded);
    } catch (error) {
...
async function authenticateUser(req, res, next) {
  try {
    ...
    try {
      ...
      provider = 'auth0';
      console.log('✅ Auth0 token verified for user:', decoded.sub);
    } catch (auth0Error) {
      try {
        // Try Firebase
        ...
        console.error('Token verification failed:', auth0Error.message, firebaseError.message);
        return res.status(401).json({
          error: 'Invalid token',
